...
export default ESLintUtils.RuleCreator(
  ...
  meta: {
    type: "problem",
    docs: {
      description: "Flow-sensitive effect detection via local call graph",
      recommended: "error"
    },
    schema: [],
    messages: {
  ...
  create(context) {
    ...
    function reportFrom(effect: Eff, node: TSESTree.Node) {
      if (effect === "eval") {
        context.report({ node, messageId: "eval" });
        return;
      }
      if (effect === "net" && !effectDeclared(env, "net", "read")) {
        context.report({ node, messageId: "net" });
      }
      if (effect === "dom" && !effectDeclared(env, "dom", "access")) {
