...
function extractTrailingHash(node: TSESTree.Node, sourceText: string): string | null {
  ...
  const match = slice.match(/asx-hash:(sha256:[a-f0-9]{64})/i);
...
export default ESLintUtils.RuleCreator(
  ...
  meta: {
    type: "problem",
    docs: {
      description: "Require hash-bound imports to include matching hash hint",
      recommended: "error"
    },
    schema: [],
    messages: {
  ...
  create(context) {
    ...
    return {
      ImportDeclaration(node) {
        const spec = (node.source as { value?: unknown })?.value;
